...
'''
Environment in which the quadcopter can only move in a 2D plane (y - z): left, right, down or up 
'''
class DroneEnv2D(gym.Env):
    ...
    def __init__(self, episode_max_steps = 300):
        ...
        # states: roll, v_y, v_z, OUT_L, OUT_R, Delta_roll, Delta_z
        low =  [-1, -1, -1, 0, 0, -1, 0]
        high = [ 1,  1,  1, 1, 1,  1, 1]
        ...
        self.skip_msg = 1 # means don't skip msgs
    ...
    def get_state(self):
        ...
        s += [self.last_odom_vel_linear[1] / self.max_horizontal_vel, 
    ...
    def calculate_reward(self):
        ...
        dist_eucl_pos = dist_eucl_pos / math.sqrt((self.horizontal_bound ** 2) + self.vertical_bound ** 2)
