...
class DroneEnv1D(gym.Env):
    """Custom Environment that follows gym interface"""
    metadata = {'render.modes': ['console']}

    def __init__(self, episode_max_steps = 300):
        super(DroneEnv1D, self).__init__()
    ...
    def calculate_reward(self):
        ...
        dist_eucl_pos = math.sqrt((self.last_odom_pos[2] - self.platform_center[2]) ** 2)
        # normalization to (0, 1)
        dist_eucl_pos = dist_eucl_pos / self.vertical_bound

        dist_eucl_vel = math.sqrt((self.last_odom_vel_linear[2]) ** 2)
        # normalization to (0, 1)
